...
var (
	...
	MoonchainL2AddressSuffix = "10001"
...
// Moonchain is a consensus engine used by L2 rollup.
type Moonchain struct {
	chainConfig         *params.ChainConfig
	moonchainL2Address  common.Address
	moonchainL2Treasury common.Address
}
...
var _ consensus.Engine = (*Moonchain)(nil)

// Author retrieves the Ethereum address of the account that minted the given
// block, who proposes the block (not the prover).
func (t *Moonchain) Author(header *types.Header) (common.Address, error) {
...
func (t *Moonchain) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header) error {
...
// the input slice).
func (t *Moonchain) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header) (chan<- struct{}, <-chan error) {
	if len(headers) == 0 {
...
func (t *Moonchain) verifyHeader(chain consensus.ChainHeaderReader, header, parent *types.Header, unixNow int64) error {
...
func (t *Moonchain) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
...
func (t *Moonchain) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
...
func (t *Moonchain) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	header := block.Header()
...
func (t *Moonchain) SealHash(header *types.Header) common.Hash {
	// Keccak(rlp(header))
	return header.Hash()
}
...
func (t *Moonchain) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
	return common.Big0
}

// ValidateAnchorTx checks if the given transaction is a valid MoonchainL2.anchor or MoonchainL2.anchorV2 transaction.
func (t *Moonchain) ValidateAnchorTx(tx *types.Transaction, header *types.Header) (bool, error) {
	if tx.Type() != types.DynamicFeeTxType {
		return false, nil
	}

	if tx.To() == nil || *tx.To() != t.moonchainL2Address {
		return false, nil
	}
...
func (t *Moonchain) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	return nil
}

// Close terminates any background threads maintained by the consensus engine.
func (t *Moonchain) Close() error {
	return nil
}
